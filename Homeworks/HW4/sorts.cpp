...
{
    // TO: You implement this.  Using the same criteria as compareStore,
...
int main()
{
    ...
    // destruction time.
    {
        ...
        // TO: Create a vector of Store pointers, and set each pointer
        //        to point to the corresponding Store in auxStores.
        vector<const Store*> StorePtrs;
        for (int i = 0; i < auxStores.size(); i++)
        {
            Store* p = &auxStores[i];
            StorePtrs.push_back(p);
        }
        
        // TO:  Sort the vector of pointers using the STL sort algorithm
        //        with compareStorePtr as the ordering relationship.
        sort(StorePtrs.begin(), StorePtrs.end(), compareStorePtr);
        
        // TO:  Using the now-sorted vector of pointers, replace each Store
